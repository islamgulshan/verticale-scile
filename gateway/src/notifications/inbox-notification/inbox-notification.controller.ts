...
export class InboxNotificationController {
    ...
    async create(@Body() dto: InboxNotificationDto, @Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('create-inbox-notification', { ...dto, user_id: request.user?._id }))
    }
    @Get("user-inbox-notification")
    async getByUser(@Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('user-inbox-notification', request.user?._id))
    }
}
