...
export class PicksNotificationController {
    ...
    async create(@Body() dto: PicksNotificationDto, @Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('create-picks-notification', { ...dto, user_id: request.user?._id }))
    }
    @Get("user-picks-notification")
    async getByUser(@Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('user-picks-notification', request.user?._id))
    }
}
