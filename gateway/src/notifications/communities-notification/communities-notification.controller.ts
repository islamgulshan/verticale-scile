...
export class CommunitiesNotificationController {
    ...
    async create(@Body() dto: CommunitiesNotificationDto, @Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('create-communities-notification', { ...dto, user_id: request.user?._id }))
    }
    @Get("user-communities-notification")
    async getByUser(@Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('user-communities-notification', request.user?._id))
    }
}
