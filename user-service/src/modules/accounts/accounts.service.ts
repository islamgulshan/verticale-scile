...
export class AccountsService {
    constructor(@InjectModel(Accounts.name) private AccountsModel: Model<Accounts>) { }
            async create(dto: Partial<Accounts>): Promise<Accounts> {
                return this.AccountsModel.findOneAndUpdate({ user_id: dto.user_id }, { ...dto }, { new: true, upsert: true });
            }
            ...
            async ActivedeActive(dto: Partial<Accounts>): Promise<Accounts> {
                return this.AccountsModel.findOneAndUpdate({user_id:dto.user_id },{...dto},{new:true});
            }
