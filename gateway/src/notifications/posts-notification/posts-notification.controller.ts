...
export class PostsNotificationController {
    ...
    async create(@Body() dto: PostNotificationDto, @Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('create-posts-notification', { ...dto, user_id: request.user?._id }))
    }
    @Get("user-posts-notification")
    async getByUser(@Req() request: any) {
        return await firstValueFrom(this.UserServiceClient.send('user-posts-notification', request.user?._id))
    }
}
