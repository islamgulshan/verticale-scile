...
export class PostsNotificationService {
  constructor(
    @InjectModel(PostsNotifaction.name)
    private PostsNotifactionModel: Model<PostsNotifaction>,
  ) {}
  async create(dto: Partial<PostsNotifaction>): Promise<PostsNotifaction> {
    return this.PostsNotifactionModel.findOneAndUpdate(
      { user_id: dto.user_id },
      { ...dto },
  ...
  async getByUser(
    user_id: Partial<PostsNotifaction>,
  ): Promise<PostsNotifaction> {
